...
def register(request):
    if request.method=="POST":
        ...
        if gender=="1":
            ...
            gender="Male"
        send_mail("Thanks for Registration","Hello Mr./Ms.{} {}\n Thanks for registering".format(first_name,last_name),"ashokbuengg@gmail.com",[email,],fail_silently=True)
        return HttpResponse("<h1>{}</br>{}</br>{}</br>{}</br>{}</br>{}</br>{}<br>{}</br>{}</br>".format(first_name,last_name,email,pwd,phno,gender,date,month,year))
